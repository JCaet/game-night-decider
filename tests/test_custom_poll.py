...
async def test_create_poll_custom_mode_shows_games(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g2 = Game(id=2, name="Wingspan", min_players=2, max_players=4, playing_time=60, complexity=2.5)
        session.add_all([g1, g2])
        await session.flush()
...
async def test_custom_poll_vote_adds_vote(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g1 = Game(id=game_id, name="TestGame", min_players=2, max_players=4, playing_time=60, complexity=2.0)
        session.add(g1)
        await session.flush()
...
async def test_custom_poll_close_announces_winner(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g1 = Game(id=1, name="Winner Game", min_players=2, max_players=4, playing_time=60, complexity=2.0)
        g2 = Game(id=2, name="Loser Game", min_players=2, max_players=4, playing_time=60, complexity=2.5)
        session.add_all([g1, g2])
        await session.flush()
...
async def test_custom_poll_close_tie(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g1 = Game(id=1, name="TieGame1", min_players=2, max_players=4, playing_time=60, complexity=2.0)
        g2 = Game(id=2, name="TieGame2", min_players=2, max_players=4, playing_time=60, complexity=2.5)
        session.add_all([g1, g2])
        await session.flush()
...
async def test_custom_poll_close_resolves_category_votes(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g1 = Game(id=base_id, name="ComplexGame1", min_players=2, max_players=4, playing_time=60, complexity=4.5)
        g2 = Game(id=base_id+1, name="ComplexGame2", min_players=2, max_players=4, playing_time=60, complexity=4.2)
        session.add_all([g1, g2])
        await session.flush()
...
async def test_custom_poll_close_with_weighted_voting(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        session.add(Session(chat_id=chat_id, is_active=True, poll_type=PollType.CUSTOM, settings_weighted=True))
        ...
        g1 = Game(id=1, name="StarredGame", min_players=2, max_players=4, playing_time=60, complexity=2.0)
        g2 = Game(id=2, name="NormalGame", min_players=2, max_players=4, playing_time=60, complexity=2.5)
        session.add_all([g1, g2])
        await session.flush()
...
# ============================================================================
from src.bot.handlers import toggle_weights_callback
...
async def test_toggle_weights_updates_setting_and_refresh_menu(mock_update, mock_context):
    ...
    _, kwargs = mock_update.callback_query.edit_message_text.call_args
    text = kwargs.get("text", "") or mock_update.callback_query.edit_message_text.call_args[0][0]
...
async def test_custom_poll_ui_grouping(mock_update, mock_context):
    ...
    import random
    chat_id = random.randint(10000, 99999)
    poll_id = f"poll_grouped_{chat_id}"
    base_id = random.randint(10000, 90000)
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        gA = Game(id=g_ids[0], name="GameA", min_players=1, max_players=5, complexity=2.0, playing_time=60)
        gB = Game(id=g_ids[1], name="GameB", min_players=1, max_players=5, complexity=4.5, playing_time=60)
        gC = Game(id=g_ids[2], name="GameC", min_players=1, max_players=5, complexity=4.2, playing_time=60)
        gD = Game(id=g_ids[3], name="GameD", min_players=1, max_players=5, complexity=1.5, playing_time=60)
        session.add_all([gA, gB, gC, gD])
        
        # User owns all (Game 0 is starred)
        state_map = {g_ids[0]: GameState.STARRED, g_ids[1]: GameState.INCLUDED, g_ids[2]: GameState.INCLUDED, g_ids[3]: GameState.INCLUDED}
...
from src.bot.handlers import custom_poll_action_callback
...
async def test_custom_poll_random_vote(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        # Group 4: Two games
        g1 = Game(id=base_id, name="Complex1", min_players=1, max_players=5, complexity=4.5, playing_time=60)
        g2 = Game(id=base_id+1, name="Complex2", min_players=1, max_players=5, complexity=4.2, playing_time=60)
        session.add_all([g1, g2])
...
async def test_custom_poll_category_vote_toggle(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        # Group 3: One game
        g1 = Game(id=base_id, name="Medium", min_players=1, max_players=5, complexity=3.5, playing_time=60)
        session.add(g1)
...
from src.bot.handlers import start_poll_callback, join_lobby_callback
...
async def test_auto_close_previous_polls(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        g1 = Game(id=random.randint(10000, 99999), name="Game1", min_players=1, max_players=5, complexity=2.0, playing_time=60)
        session.add(g1)
...
async def test_auto_refresh_poll_on_join(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        # Games
        g1 = Game(id=random.randint(10000, 99999), name="GameA", min_players=1, max_players=5, complexity=2.0, playing_time=60)
        session.add(g1)
        session.add(Collection(user_id=111, game_id=g1.id))
        session.add(SessionPlayer(session_id=chat_id, user_id=111))
    ...
    calls = mock_context.bot.edit_message_text.call_args_list
    # Should be at least 2 calls: one for lobby update (no message_id arg, usually) or on query.message
