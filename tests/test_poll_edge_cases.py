...
from src.core.models import Session, User, Game, Collection, SessionPlayer, GameNightPoll, PollVote, GameState, PollType
from src.bot.handlers import custom_poll_vote_callback, custom_poll_action_callback, render_poll_message, cancel_night_callback
...
async def test_anonymous_voting_toggle(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        session.add(Session(chat_id=chat_id, is_active=True, poll_type=PollType.CUSTOM, hide_voters=False))
    ...
    # Based on buttons "toggle_weights", "toggle_hide_voters" usually handled in specific callback or general 'poll_settings'
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        await render_poll_message(mock_context.bot, chat_id, 999, session, poll_id, games, priority_ids)
    ...
    # Should confirm anonymity
    assert "🙈 Anonymous" in text or "Anonymous" in text 
...
async def test_vote_on_non_existent_game(mock_update, mock_context):
    ...
    try:
        ...
        # If it raises IntegrityError, that's one way to handle it.
        # But ideally we probably want to catch it or ignore.
        # Let's iterate: if logic checks existence?
        pass

    # Verify no vote added or crash handled
    async with db.AsyncSessionLocal() as session:
        votes = (await session.execute(select(PollVote))).scalars().all()
        # If FK constraint active, it failed. If not, maybe added?
        # Ideally code should handle it.
        # Check handlers.py custom_poll_vote_callback logic... it likely doesn't check Game existence first.
...
async def test_render_anonymous_poll(mock_update, mock_context):
    ...
    async with db.AsyncSessionLocal() as session:
        session.add(Session(chat_id=chat_id, is_active=True, poll_type=PollType.CUSTOM, hide_voters=True))
        session.add(User(telegram_id=222, telegram_name="SecretVoter"))
        
        g1 = Game(id=2, name="SecretGame", min_players=1, max_players=5, complexity=3.0, playing_time=45)
        session.add(g1)
